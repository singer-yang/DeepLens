...
class PSFNet_MLPConv3(nn.Module):
    """
    ...
    def __init__(
        ...
        super(PSFNet_MLPConv3, self).__init__()
...
def measure_gflops(model, dummy_input):
    """Measures GFLOPs and number of parameters of a model."""
    try:
        from thop import profile

        macs, params = profile(model, inputs=(dummy_input,), verbose=False)
        gflops = (macs * 2) / 1e9
        return gflops, params / 1e6  # Return GFLOPs and params in millions
    except ImportError:
...
if __name__ == "__main__":
    ...
    model_path = "psfnet_mplconv3_model.pth"
    ...
    # Measure GFLOPs
    print("\n--- Model Complexity ---")
    # Use a single batch item for FLOPs calculation
    dummy_input_for_flops = torch.tensor([[0.5, -5000.0]])
    gflops, params_m = measure_gflops(model, dummy_input_for_flops)
