...
class Anamorphic(Surface):
    ...
    def __init__(self, r, d, c_x, c_y, mat2, squeeze_factor=1.5, base_focal_length=50.0, device="cpu"):
        ...
        Args:
            ...
            cy (float): Curvature in y-direction (vertical)
        ...
        if c_x is None or c_y is None:
            ...
            self.c_y = torch.tensor(1.0 / base_focal_length, device=device)  # Vertical curvature
    ...
    def init_from_dict(cls, surf_dict):
        ...
        if "roc_x" in surf_dict:
            ...
            c_x = 1 / surf_dict["c_x"]
        if "roc_y" in surf_dict:
            c_y = 1 / surf_dict["roc_y"]
        else:
            c_y = 1 / surf_dict["c_y"]
        
        return cls(r=surf_dict["r"], d=surf_dict["d"], c_x=c_x, c_y=c_y, mat2=surf_dict["mat2"], 
    ...
    def perturb(self, tolerance):
        ...
        self.cy_offset = np.random.randn() * tolerance.get("cy", 0.001)
        # Apply the perturbations if desired:
        self.c_x = self.c_x + self.cx_offset
        self.c_y = self.c_y + self.cy_offset
    ...
    def get_optimizer_params(self, lr=[0.001, 0.001, 0.001], optim_mat=False):
        ...
        self.c_y.requires_grad_(True)
        self.d.requires_grad_(True)

        params = []
        params.append({"params": [self.c_x], "lr": lr[0]})
        params.append({"params": [self.c_y], "lr": lr[1]})
