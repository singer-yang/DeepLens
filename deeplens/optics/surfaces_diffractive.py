...
class DOE(DeepObj):
    def __init__(self, l, d, res=None, fab_ps=0.001, param_model='pixel2d', device=DEVICE):
        ...
        self.n0 = self.refractive_index(self.wvln0) # refractive index of the substrate at design wavelength
    ...
    def get_pmap(self):
        ...
        if self.param_model == 'fresnel':
            pmap = - 2 * np.pi * torch.fmod((self.x**2 + self.y**2) / (2 * 0.55e-3 * self.f0), 1) # unit [mm]
            ...
            pmap = self.order2 * r**2 + self.order4 * r**4 + self.order6 * r**6 + self.order8 * r**8

        elif self.param_model == 'poly1d':
            pmap_even = self.order2 * r**2 + self.order4 * r**4 + self.order6 * r**6
            pmap_odd =  self.order3 * (x_norm**3 + y_norm**3) + self.order5 * (x_norm**5 + y_norm**5) + self.order7 * (x_norm**7 + y_norm**7)
    ...
    def refractive_index(self, wvln=0.55):
        ...
        if self.glass == 'fused_silica':
            assert wvln >= 0.4 and wvln <= 0.7, 'Wavelength should be in the range of [0.4, 0.7] um.'
            ref_wvlns = [0.40, 0.41, 0.42, 0.43, 0.44, 0.45,
            ...
            ref_n = [1.4701, 1.4692, 1.4683, 1.4674, 1.4665, 1.4656, 
                     1.4649, 1.4642, 1.4636, 1.4629, 1.4623, 1.4619, 
                     1.4614, 1.4610, 1.4605, 1.4601, 1.4597, 1.4593, 
                     1.4589, 1.4585, 1.4580, 1.4577, 1.4574, 1.4571, 
                     1.4568, 1.4565, 1.4563, 1.4560, 1.4558, 1.4555, 
                     1.4553]
            ...
            # Interpolate n
            n = ref_n[idx1] + (ref_n[idx2] - ref_n[idx1]) / (ref_wvlns[idx2] - ref_wvlns[idx1]) * (wvln - ref_wvlns[idx1])
    ...
    def draw_phase_map3d(self, save_name='./DOE_phase_map3d.png'):
        ...
        ax.scatter(X.flatten(), Y.flatten(), pmap.cpu().numpy().flatten(), marker='.', s=0.01, c=pmap.cpu().numpy().flatten(), cmap='viridis')
        ax.set_aspect('equal')
        ax.axis('off')
        fig.savefig(save_name, dpi=600, bbox_inches='tight')
        plt.close(fig)
...
class ThinLens(DeepObj):
    ...
    def forward(self, field):
        ...
        phi_lens = torch.fmod(- field.k * (field.x ** 2 + field.y ** 2) / (2 * foclen), 2 * np.pi)
...
def Zernike(z_coeff, r, alpha):
    ...
    Z16 =  z_coeff[15] * math.sqrt(12) * (10 * r**5 - 12 * r**3 + 3 * r) * torch.cos(alpha)
    Z17 =  z_coeff[16] * math.sqrt(12) * (10 * r**5 - 12 * r**3 + 3 * r) * torch.sin(alpha)
    ...
    Z23 =  z_coeff[22] * math.sqrt(14) * (15 * r**6 - 20 * r**4 + 6 * r**2) * torch.sin(2 * alpha)
    Z24 =  z_coeff[23] * math.sqrt(14) * (15 * r**6 - 20 * r**4 + 6 * r**2) * torch.cos(2 * alpha)
