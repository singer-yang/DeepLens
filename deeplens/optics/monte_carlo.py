...
def forward_integral(ray, ps, ks, pointc_ref=None, coherent=False):
    ...
    psf_range = [
        (-ks / 2 + 0.5) * ps,
        (ks / 2 - 0.5) * ps,
    ]  # this ensures the pixel size doesnot change in assign_points_to_pixels function

    # ==> PSF center
    if pointc_ref is None:
        # Use RMS center
        pointc = (points * ray.ra.unsqueeze(-1)).sum(0) / ray.ra.unsqueeze(-1).sum(
            0
        ).add(EPSILON)
    ...
    ra = (
        ray.ra
        * (points_shift[..., 0].abs() < (psf_range[1] - 0.1 * ps))
        * (points_shift[..., 1].abs() < (psf_range[1] - 0.1 * ps))
    )  # shape [spp, N] or [spp].
    ...
    # Coherent ray tracing, integral over complex amplitude
        ...
        if single_point:
            ...
            phase = torch.fmod((opl - opl.min(0).values) / (ray.wvln * 1e-3), 1) * (
                2 * np.pi
            )  # [spp, N] to [spp]
            ...
            for i in range(ray.o.shape[1]):
                ...
                obliq = ray.d[
                    :, i, 2
                ]  # from [spp, N, 3] to [spp], for amplitude correction.
                opl = ray.opl[:, i]  # from [spp, N] to [spp]
                phase = torch.fmod((opl - opl.min(0).values) / (ray.wvln * 1e-3), 1) * (
                    2 * np.pi
                )  # [spp, N] to [spp]

                psf_u = assign_points_to_pixels(
                    points=points_shift0,
                    ks=ks,
                    x_range=psf_range,
                    y_range=psf_range,
    ...
    if interpolate:
        ...
        if coherent:
            ...
            grid.index_put_(
                ...
                (1 - w_b) * (1 - w_r) * ra * obliq * torch.exp(1j * phase),
                accumulate=True,
            )
            grid.index_put_(
                tuple(pixel_indices_tr.t()),
                (1 - w_b) * w_r * ra * obliq * torch.exp(1j * phase),
                accumulate=True,
            )
            grid.index_put_(
                tuple(pixel_indices_bl.t()),
                w_b * (1 - w_r) * ra * obliq * torch.exp(1j * phase),
                accumulate=True,
            )
            grid.index_put_(
                tuple(pixel_indices_br.t()),
                w_b * w_r * ra * obliq * torch.exp(1j * phase),
                accumulate=True,
            )
            ...
            grid.index_put_(
                tuple(pixel_indices_br.t()), w_b * w_r * ra * obliq, accumulate=True
            )
