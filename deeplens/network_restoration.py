...
1, Onelayer, ImageRestorationNet, SeeInDark. Only for debugging.
...
# ====================================================================================
# Simple network for debugging
# ====================================================================================

class Onelayer(nn.Module):
    def __init__(self, c=3):
        super(Onelayer, self).__init__()
        self.conv = nn.Conv2d(c, c, kernel_size=1, stride=1)
...
class ImageRestorationNet(nn.Module):
    def __init__(self):
        ...
        # Define the layers
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, padding=1)  # Assuming input images are RGB
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3, padding=1)
        self.conv3 = nn.Conv2d(64, 3, kernel_size=3, padding=1)  # Output is RGB image
...
class SeeInDark(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.clamp(conv10+x, min=0, max=1)
...
class NAFNet(nn.Module):
    def __init__(self, in_chan=3, out_chan=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[]):
        super().__init__()

        self.intro = nn.Conv2d(in_channels=in_chan, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=True)
        self.ending = nn.Conv2d(in_channels=width, out_channels=out_chan, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=True)
